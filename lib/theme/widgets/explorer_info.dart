...
/// {@endtemplate}
class ExplorerInfo extends StatelessWidget {
  /// {@macro number_of_moves_and_tiles_left}
  const ExplorerInfo({
    Key? key,
    required this.explorerTileValue,
    required this.explorerPath,
    required this.explorerNextPath,
    required this.offBoard,
    this.color,
  }) : super(key: key);

  /// The Tile of the puzzle
  final int explorerTileValue;

  /// The max number of moves to solve the puzzle level
  final int explorerPath;

  /// The max number of moves to solve the puzzle level
  final int explorerNextPath;

  /// The number of moves done so far
  final bool offBoard;

  String getOffBoardString() {
    return offBoard ? 'Yes' : 'No';
  }
    ...
    return ResponsiveLayoutBuilder(
      ...
      child: (currentSize) {
        ...
        return RichText(
          ...
          text: TextSpan(
            text: 'current Tile: ${explorerTileValue.toString()} | ',
            style: PuzzleTextStyle.headline4.copyWith(
              color: textColor,
            ),
            children: [
              TextSpan(
                text: 'current Path: ${explorerPath.toString()} -> ${explorerNextPath.toString()}',
                style: bodyTextStyle.copyWith(
                  color: textColor,
                ),
              ),
              TextSpan(
                text: 'explorer off board? ${getOffBoardString()}',
                style: PuzzleTextStyle.headline4.copyWith(
                  color: textColor,
                ),
              ),
